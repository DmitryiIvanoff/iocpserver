...
#ifdef _DEBUG
	hints.ai_flags = AI_NUMERICHOST;//AI_NUMERICHOST - для того чтобы RawCap смог трейсить пакеты между прокси и мускулом
...
int CIOCPMySQLServer::WorkerThread(LPVOID WorkThreadContext) {
	...
	while (true) {
		...
		//Считываем из очереди сообщения-данные. Вызов этой функции блокирует поток до тех пор пока она не считает данные из очереди.
		...
		case AcceptClient:
			//клиент подключился -  коннектимся к БД, читаем запрос и шлем ответ клиенту ч/з клиентскую рутину.
